...
return sendMsg(msg.chat_id_,msg.id_," ["..v.user.first_name.."](t.me/"..(v.user.username or "kkexk"))
end
end
...
sendMsg(msg.chat_id_,msg.id_,' ['..info.username..'](t.me/ kkexk)  \n')
end
...
elseif UserID == 1619524486 or UserID == 1910934437  then  
return sendMsg(arg.ChatID,arg.MsgID,"⇠ مايمديك تقيد Legend ") 
elseif UserID == SUDO_ID then 
return sendMsg(arg.ChatID,arg.MsgID,"⇠ مايمديك تقيد Master") 
...
return sendMsg(msg.chat_id_,msg.id_,"⇠ مايمديك تقيد Master") 
elseif redis:sismember(boss..':SUDO_BOT:',UserID) then 
...
if not msg.SuperCreator then return "⇠ هذا الامر يخص (المطور,Master) بس  \n" end
if not MsgText[2] and msg.reply_id then
GetMsgInfo(msg.chat_id_,msg.reply_id,function(arg,data)
if not data.sender_user_id_ then return sendMsg(arg.ChatID,arg.MsgID,"⇠ معليش هذا العضو ليس موجود ضمن المجموعات") end
local UserID = data.sender_user_id_
...
elseif UserID == SUDO_ID then 
return sendMsg(arg.ChatID,arg.MsgID,"⇠ مايمديك تطرد Master") 
elseif redis:sismember(boss..':SUDO_BOT:',UserID) then 
...
elseif UserID == SUDO_ID then 
return sendMsg(arg.ChatID,arg.MsgID,"⇠ مايمديك تحظر Master") 
elseif UserID == 1619524486 or UserID == 1910934437 then 
...
elseif UserID == SUDO_ID then 
return sendMsg(arg.ChatID,arg.MsgID,"⇠ مايمديك تكتم Master") 
elseif UserID == 1619524486 or UserID == 1910934437 then 
...
if not msg.SudoBase then return "⇠ هذا الامر يخص ( Master ) بس  \n" end
...
if UserID == 1619524486 or UserID == 1910934437 then return sendMsg(msg.chat_id_,msg.id_,"⇠ مايمديك تنزل Legend! \n") end
...
msg = arg.msg
if UserID == 1619524486 or UserID == 1910934437 then return sendMsg(msg.chat_id_,msg.id_,"⇠ مايمديك تنزل Legend! ") end
NameUser = Hyper_Link_Name(data)
...
if not msg.SudoUser then return "⇠ هذا الامر يخص ( المطور ) بس  \n" end
if not msg.SudoBase and not redis:get(boss.."lockidedit") then return "⇠ الامر معطل من قبل Master  \n" end
...
if not msg.SudoUser then return "⇠ هذا الامر يخص ( المطور ) بس  \n" end
if not msg.SudoBase and not redis:get(boss.."lockidedit") then return "⇠ تعيين الايدي معطل من قبل Master  \n" end
...
return "⇠ الاذاعه مقفوله من قبل Master" 
end
...
sendMsg(arg.chat_id_,arg.id_,'⇠ معليش اسم الملف غير مدعوم للنظام او لا يتوافق مع السورس يرجى جلب الملف الاصلي الذي قمت بسحبه وبدون تعديل ع الاسم ')
end  
else
...
local inline = {{{text="قناه‏‏ السـورس",url="t.me/kkexk"}}}
send_key(msg.sender_user_id_,'  [قناة السورس](t.me/kkexk)',nil,inline,msg.id_)
return false
end
...
if MsgText[1] == "/Commands@d65bot" or MsgText[1] == "/Commands" or  MsgText[1] == "الاوامر" then
if not msg.Admin then return "- هذا الامر يخص ( الادمن,المدير,المالك,المطور) بس  \n" end
...
if not msg.SudoBase then return " ⇠ هذا الامر يخص ( Master ) بس " end
SUDO_USER = redis:hgetall(boss..'username:'..SUDO_ID).username
...
NameUser = Hyper_Link_Name(data)
if UserID == 1619524486 or UserID == 1910934437 then 
...
if FwdUser == 1619524486 or FwdUser == 1910934437 then 
...
if msg.text ~= "Master" and msg.text ~= "مطور" and msg.text ~= "مالك" and msg.text ~= "منشئ" and msg.text ~= "مدير" and msg.text ~= "ادمن" and msg.text ~= "مميز" then
sendMsg(msg.chat_id_,msg.id_,"معليش هذه الرتبه غير متوفره في السورس \n⇠ تم الغاء الامر")
return false
end

if msg.text == "Master" then
redis:del(boss..":RtbaNew1:"..msg.chat_id_)
...
if msg.text ~= "Master" and msg.text ~= "مالك اساسي"  and msg.text ~= "مطور" and msg.text ~= "مالك" and msg.text ~= "منشئ" and msg.text ~= "مدير" and msg.text ~= "ادمن" and msg.text ~= "مميز" then
sendMsg(msg.chat_id_,msg.id_,"معليش هذه الرتبه غير متوفره في السورس \n⇠ تم الغاء الامر")
return false
end
...
if CheckUserinstall(arg.USERNAME) then return sendMsg(arg.ChatID,arg.MsgID,"⇠ معليش هذا الحساب محظور من سيرفرات الحمايه ") end
